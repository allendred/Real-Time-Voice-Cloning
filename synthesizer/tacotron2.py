...
class Tacotron2:
    def __init__(self, checkpoint_path, hparams, gta=False, model_name="Tacotron"):
        log(f"Constructing model: {model_name}")
    ...
    def synthesize(self, texts, basenames, out_dir, log_dir, mel_filenames, embed_filenames):
        ...
        for i, mel in enumerate(mels):
            ...
            mel_filename = os.path.join(out_dir, f"mel-{basenames[i]}.npy")
            ...
            if log_dir is not None:
                ...
                audio.save_wav(
                    wav,
                    os.path.join(log_dir, f"wavs/wav-{basenames[i]}-mel.wav"),
                ...
                plot.plot_alignment(
                    alignments[i],
                    os.path.join(log_dir, f"plots/alignment-{basenames[i]}.png"),
                    title=f"{texts[i]}",
                    split_title=True,
                ...
                plot.plot_spectrogram(
                    mel,
                    os.path.join(log_dir, f"plots/mel-{basenames[i]}.png"),
                    title=f"{texts[i]}",
                    split_title=True,
                )
                ...
                if hparams.predict_linear:
                    ...
                    audio.save_wav(
                        wav,
                        os.path.join(
                            log_dir, f"wavs/wav-{basenames[i]}-linear.wav"
                        ),
                        sr=hparams.sample_rate,
                    )
                    ...
                    plot.plot_spectrogram(
                        linears[i],
                        os.path.join(log_dir, f"plots/linear-{basenames[i]}.png"),
                        title=f"{texts[i]}",
                        split_title=True,
                        auto_aspect=True,
                    )
    ...
    def _get_output_lengths(self, stop_tokens):
        return [row.index(1) for row in np.round(stop_tokens).tolist()]
